package com.gabriaum.skills.util.inventory.content;
...
public interface InventoryContents {
    ...
    class Impl implements InventoryContents {
        ...
        public InventoryContents set(int index, ClickableItem item) {
            ...
            int row = index / inv.getColumns();
            int column = index % inv.getColumns();
        ...
        @Override
        public List<SlotPos> getBorders() {

            List<SlotPos> positions = new ArrayList<>();
            int toRow = inv.getRows() - 1, toColumn = inv.getColumns() - 1;

            for (int row = 0; row <= toRow; row++) {
                for (int column = 0; column <= toColumn; column++) {
                    if (row != 0 && row != toRow && column != 0 && column != toColumn)
                        continue;
