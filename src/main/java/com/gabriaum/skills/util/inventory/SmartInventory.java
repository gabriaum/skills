...
import com.gabriaum.skills.util.inventory.opener.InventoryOpener;
...
public class SmartInventory {
    ...
    private boolean closeable, dynamic;

    private InventoryProvider provider;
    private SmartInventory parent;

    private List<InventoryListener<? extends Event>> listeners;
    private final InventoryController manager;

    private SmartInventory(InventoryController manager) {
        this.manager = manager;
    }

    public Inventory open(Player player) {
        return open(player, 0, null, null);
    }

    public Inventory open(Player player, int page) {
        return open(player, page, null, null);
    }

    public Inventory open(Player player, int page, String title) {
        return open(player, page, title, null);
    }


    public Inventory open(Player player, String title) {
        return open(player, 0, title, null);
    }

    public Inventory open(Player player, Map<String, Object> defaultProperties) {
        return open(player, 0, null, defaultProperties);
    }

    public Inventory open(Player player, int page, String title, Map<String, Object> defaultProperties) {
        Optional<SmartInventory> oldInv = this.manager.getInventory(player);

        AtomicReference<Map<String, Object>> properties = new AtomicReference<>();
        ...
        oldInv.ifPresent(inv -> {
            ...
            inv.manager.getContents(player).ifPresent(contents -> properties.set(contents.properties()));
        ...
        if (properties.get() != null)
            contents.properties(properties.get());

        if (defaultProperties != null) {
            contents.properties().putAll(defaultProperties);
        ...
        try {
            ...
            Inventory handle = opener.open(this, player, title);
    ...
    public InventoryController getManager() {
        return manager;
    }
    ...
    public static final class Builder {
        ...
        private InventoryController manager;
        private InventoryProvider provider;
        private SmartInventory parent;
        ...
        public Builder manager(InventoryController manager) {
            this.manager = manager;
            return this;
        }
        ...
        public SmartInventory build() {
            ...
            InventoryController manager = this.manager != null ? this.manager : SkillsMain.getInstance().getInventoryController();
